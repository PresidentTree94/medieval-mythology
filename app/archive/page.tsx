...
export default async function Archive() {
  ...
  const inspirationData = (await sb.getInspirations()).filter(i => i.status);
  ...
  const books: Record<string, Book<any>> = {
    ...
    pantheon: {
      headings: ["Epithet", "Inspiration", "Domains"],
  ...
  return (
    <main>
      <Hero className="h-[520px] items-end" img="archive">
        ...
        <p className="text-xs md:text-sm font-display tracking-[0.5em] uppercase text-[oklch(0.84_0.115_84)]">{total} Entries Across Five Books</p>
        <h1 className="text-4xl sm:text-5xl md:text-6xl text-card mt-4 mb-5">The Grand Archive</h1>
        <p className="max-w-2xl mx-auto md:text-lg text-background-light italic">A complete index of every entry across the five books of the codex — characters, kingdoms, pantheon, and myths.</p>
      </Hero>
